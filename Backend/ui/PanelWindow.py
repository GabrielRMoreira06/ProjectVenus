...
class MoodStatBar(QWidget):
    ...
    def __init__(self, icon_name, label):
        ...
        name_label.setStyleSheet(f"color: {PINK_SOFT}; font-size: 11px; border: none; background: transparent;")
...
class ChatBubble(QFrame):
    """
    One message bubble. side='left' (Venus) or 'right' (user).
    image, if given (a PIL.Image), is shown as a small thumbnail above
    the text. text may be empty when a message was image-only.
    """
    ...
    def __init__(self, text, timestamp, side="left", image=None):
        ...
        if image is not None:
            ...
            thumbnail_label.setStyleSheet(f"border: 1px solid {PINK}; border-radius: 12px;")
            column.addWidget(
                thumbnail_label,
...
class PanelWindow(QWidget):
    ...
    def _build_sidebar(self):
        ...
        divider.setStyleSheet(f"background-color: {PINK};")
        layout.addSpacing(4)
    ...
    def _build_profile_section(self):
        """
        NOTE: name/Level/XP are static placeholders — there's no
        leveling system in the backend to source real numbers from.
        The mood bars below this are real (see _refresh_mood).
        """
        container = QFrame()
        ...
        level_label = QLabel("Lv. —")  # placeholder, no leveling system yet
        ...
        xp_value = QLabel("— / —")  # placeholder, no leveling system yet
        ...
        # Real mood values — see _refresh_mood().
    ...
    def _build_main_area(self):
        ...
        self.content_stack.addWidget(PlaceholderPage("Preferences"))
